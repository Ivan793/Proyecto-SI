...
class GuestService:
    ...
    async def create_guest_with_user(self, guest_data: GuestCreateWithUser) -> GuestResponse:
        ...
        if existing_user:
            raise UserAlreadyExistsException(f"El usuario con correo {guest_data.correo} ya existe.")
        ...
        # Crear invitado con el id_usuario recién creado
        guest_dict = {
    ...
    async def create_guest_with_existing_user(self, guest_data: GuestCreateWithExistingUser) -> GuestResponse:
        ...
        # Verificar que el invitado no exista para ese usuario
    ...
    # ---------------------------
    # Obtener todos los invitados (paginado)
    # ---------------------------
    async def get_all_guests(self, active_only: bool = True, page: int = 1, limit: int = 20) -> tuple[List[GuestResponse], int]:
    ...
    # ---------------------------
    async def get_guest_with_user(self, guest_id: str) -> GuestResponse:
        guest = await self.guest_repo.get_by_id(guest_id)
        if not guest:
            raise GuestNotFoundException(f"Invitado con ID {guest_id} no encontrado.")
        ...
        if not user:
            raise UserNotFoundException(f"Usuario del invitado con ID {guest['id_usuario']} no encontrado.")

        guest_with_user = {
            "invitado": GuestResponse(**guest),
            "usuario": user
        }
        return guest_with_user
    ...
    async def update_guest(self, guest_id: str, guest_data: GuestUpdate) -> GuestResponse:
        ...
        updated_dict = guest_data.model_dump(exclude_unset=True)
        updated_dict["updated_at"] = datetime.utcnow()
        updated_guest = await self.guest_repo.update(guest_id, updated_dict)
    ...
    async def deactivate_guest(self, guest_id: str, reason: str) -> bool:
        ...
        updated_guest = await self.guest_repo.update(
            guest_id,
